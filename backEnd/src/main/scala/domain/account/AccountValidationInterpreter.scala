...
object AccountValidationInterpreter {
  ...
  private val checkPassword = checkPred(
    longerThan(6)("Password") and
      Predicate.lift(error("The password must contain at least an uppercase letter"),   str => str.exists(_.isUpper)) and
      Predicate.lift(error("The password must  contain at least a lowercase letter"),   str => str.exists(_.isLower)) and
      Predicate.lift(error("The password must  contain at least a symbol or a number"), str => str.exists(!_.isLetter))
  )
