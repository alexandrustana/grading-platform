...
class AssignmentEndpoints[F[_]: Effect] extends Http4sDsl[F] {
  ...
  implicit val assignmentDecoder: EntityDecoder[F, Assignment] =
    jsonOf[F, Assignment]
  implicit val assignmentListDecoder: EntityDecoder[F, List[Assignment]] =
    jsonOf[F, List[Assignment]]

  private def createAssignment(
      assignmentService: AssignmentService[F]): HttpService[F] =
    HttpService[F] {
