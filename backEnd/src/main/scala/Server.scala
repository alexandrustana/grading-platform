...
object Server extends StreamApp[IO] {
  ...
  def createStream[F[_]](implicit E: Effect[F]): Stream[F, ExitCode] =
    for {
      ...
      accountRepo          = accountRepository[F](Left(xa))
      professorRepo        = professorRepository[F](Left(xa))
      studentRepo          = studentRepository[F](Left(xa))
      courseRepo           = courseRepository[F](Left(xa))
      assignmentRepo       = assignmentRepository[F](Left(xa))
      submissionRepo       = submisssionRepository[F](Left(xa))
